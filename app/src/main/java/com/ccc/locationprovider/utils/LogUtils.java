...
public final class LogUtils {
    ...
    private static int sBuglyFilter = V;     // log bugly过滤器
    private static int sFileFilter = V;     // log 文件过滤器
    private static int sStackDeep = 1;     // log 栈深度
    private static final String BUGLY_TAG = "SHIJIA_LOGCAT";
    ...
    //bugly日志 start
    public static void vBugly(final String contents) {
        bugly(V, BUGLY_TAG, contents);
    }

    public static void vBuglyTag(final String tag, final String contents) {
        bugly(V, tag, contents);
    }

    public static void dBugly(final String contents) {
        bugly(D, BUGLY_TAG, contents);
    }

    public static void dBuglyTag(final String tag, final String contents) {
        bugly(D, tag, contents);
    }

    public static void iBugly(final String contents) {
        bugly(I, BUGLY_TAG, contents);
    }

    public static void iBuglyTag(final String tag, final String contents) {
        bugly(I, tag, contents);
    }

    public static void wBugly(final String contents) {
        bugly(W, BUGLY_TAG, contents);
    }

    public static void wBuglyTag(final String tag, final String contents) {
        bugly(W, tag, contents);
    }

    public static void eBugly(final String contents) {
        bugly(E, BUGLY_TAG, contents);
    }

    public static void eBuglyTag(final String tag, final String contents) {
        bugly(E, tag, contents);
    }

    private static void bugly(@TYPE final int type, String tag, final String content) {
//        int type_low = type & 0x0f;
//        if (type_low < sBuglyFilter) return;
...
//        }
    ...
    public static String getLogPath(Context context) {
        return sDir == null ? sDefaultDir : sDir;
    }

    public static File getLogPathFile(Context context) {
        return new File(sDir == null ? sDefaultDir : sDir);
    }


    public static boolean clearOutOfDate(final long time) {
        if (sExecutor == null) {
        ...
        Future<Boolean> submit = sExecutor.submit(new Callable<Boolean>() {
            ...
            public Boolean call() throws Exception {
                File path = new File((sDir == null ? sDefaultDir : sDir));
                File[] files = path.listFiles();
                if (files != null) {
                    for (File f : files) {
                        if ((System.currentTimeMillis() - f.lastModified()) > time) {
                            Log.i("locallog", "time is " + (System.currentTimeMillis() - f.lastModified()));
                            f.delete();
                            return true;
                        }
                    }
    ...
    public static boolean deleteFile(final ArrayList<File> files) {
        if (sExecutor == null) {
            sExecutor = Executors.newSingleThreadExecutor();
        }
        Future<Boolean> submit = sExecutor.submit(new Callable<Boolean>() {
    ...
    public static class Config {
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                ...
                sDefaultDir = UtilsApp.getApp().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                sDefaultDir = UtilsApp.getApp().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
        ...
        public Config setBuglyFilter(@TYPE final int buglyFilter) {
            sBuglyFilter = buglyFilter;
            return this;
        }
